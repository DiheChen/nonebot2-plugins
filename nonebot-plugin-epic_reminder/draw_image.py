"""
 ...
 - LastEditTime : 2021-11-15 18:47:15
...
class GenerateImage:
    def __init__(self, data: list) -> None:
        self._data = data
        self._urls = [i["url"] for i in data]
        self._session = ClientSession()
        self._font_path = path.abspath(
            path.join(path.dirname(__file__), "msyh.ttc"))

    async def _get(self) -> Generator[Image.Image, None, None]:
        for url in self._urls:
            async with self._session.get(url, verify_ssl=False) as resp:
                yield Image.open(BytesIO(await resp.read())).convert("RGBA").resize((360, 480))

    async def __aenter__(self) -> "GenerateImage":
        self.images: List[Image.Image] = list()
        async for image in self._get():
            self.images.append(image)
        return self

    @staticmethod
    def date2str(date: str) -> str:
    ...
    async def generate_image(self) -> str:
        image = Image.new(
            "RGBA", (len(self.images) * 480 - abs(len(self.images) - 1 * 40) if len(self._data)
                                                                                > 1 else 460, 720),
            (18, 18, 18, 255))
        ...
        for count, game_cover in enumerate(self.images):
            banner_draw.rounded_rectangle((0, 0, 360, 40), 10, fill=(
                63, 72, 204) if time() > date2time_stamp(self._data[count]["start_date"]) else (0, 0, 0))
            ...
            image = self.draw_text(image, "当前免费" if time() > date2time_stamp(
                self._data[count]["start_date"]) else "即将推出", 20, (count * 440 + 205, 535))
            image = self.draw_text(
                image, self._data[count]["title"], 20, (count * 440 + 60, 585))
            image = self.draw_text(
                image, "原价" + " " * 9 + self._data[count]["original_price"], 20, (count * 440 + 60, 620))
            image = self.draw_text(
                image, "开始时间:  " +
                       GenerateImage.date2str(self._data[count]["start_date"]),
                20, (count * 440 + 60, 655))
            image = self.draw_text(
                image, "截止时间:  " +
                       GenerateImage.date2str(self._data[count]["end_date"]),
                20, (count * 440 + 60, 680))
        return GenerateImage.image2b64(image)
